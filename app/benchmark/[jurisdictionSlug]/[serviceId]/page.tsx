...
type CorrectionsDocument = { corrections?: Correction[]; runs?: Array<{ runId?: string; date?: string; description?: string; correctionIds?: string[]; file?: string; data?: { corrections?: Correction[]; auditDate?: string } }> };
...
function correctionRuns(document: CorrectionsDocument) {
  if (document.corrections && document.runs?.length) {
    const correctionById = new Map(document.corrections.map((correction) => [correction.id, correction]));
    const grouped = document.runs.map((run) => ({ label: [run.date, run.description ?? run.runId].filter(Boolean).join(' · '), corrections: (run.correctionIds ?? []).map((id) => correctionById.get(id)).filter((correction): correction is Correction => Boolean(correction)) }));
    const groupedIds = new Set(document.runs.flatMap((run) => run.correctionIds ?? []));
    const remaining = document.corrections.filter((correction) => !groupedIds.has(correction.id));
    return remaining.length ? [...grouped, { label: 'Other audit corrections', corrections: remaining }] : grouped;
  }
  if (document.corrections) return [{ label: 'Audit corrections', corrections: document.corrections }];
  return (document.runs ?? []).map((run) => ({ label: run.data?.auditDate ?? run.file ?? 'Audit corrections', corrections: run.data?.corrections ?? [] }));
}
...
export default async function EvidencePage({ params }: { params: Promise<Params> }) {
  ...
  const corrections = correctionRuns(evidence.corrections);
  ...
  return <main className="report-page evidence-page" data-stated-count={evidence.scorecard.statedCount}>
